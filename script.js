// DOM ELEMENTS
 ...
 const startGameBtn = document.getElementById("instruction-btn");
...
const players = {
    playerOne:{name:"Steve", wins: 0},
    playerTwo:{name:"Jack", wins: 0}
}
...
else{nextPlayer = players.playerTwo.name;
...
function checkForWin(){
    ...
    for(const line of lines){
        ...
        if(hasCross || hasCircle){
            ...
            winner.wins += 1;
           if(playerOneScore && playerTwoScore){ updateScores();
           }
           playerWon()
            return;
        }
...
function  updateScores(){
   if(playerOneScore && playerTwoScore){playerOneScore.innerHTML = players.playerOne.wins
...
function checkForTie(){
    console.log("Entering for checkForTie function");
    const squares = [
        squareOne,
        squareTwo,
        squareThree,
    ...
    if(allSquaresFilled && !playerHasWon){
        infoText.innerHTML =   `Game is a tie!`;
        continueGame();
    }
}

// CONTINUE,RESTART OR RESET GAME 
